...
export const authService = {
  ...
  async resetPassword(correo: string, contrasenaAnterior: string, contrasenanueva: string): Promise<void> {
  ...
  async createUsuario(nombreCompleto: string, correo: string, nombreUsuario: string, contrasena: string, idRol: number): Promise<UsuarioResponse> {
  ...
  async updateUsuario(id: number, nombreCompleto: string, correo: string, nombreUsuario: string): Promise<UsuarioResponse> {
    const response = await fetch(`${API_BASE}/usuarios/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
